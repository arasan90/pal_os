...
PAL_OS_RAM_ATTR int pal_queue_enqueue_from_isr(pal_queue_t *queue, void *const item, size_t timeout_ms)
{
	int ret_code = -1;
	if (queue && item)
	{
...
PAL_OS_RAM_ATTR int pal_queue_dequeue_from_isr(pal_queue_t *queue, void *const item, size_t timeout_ms)
{
	int ret_code = -1;
	if (queue && item)
	{
