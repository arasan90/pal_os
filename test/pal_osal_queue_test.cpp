...
TEST(pal_os_queue, DequeAlreadyFilledQueueNoTimeout)
{
	...
	EXPECT_EQ(0, pal_queue_create(&queue, sizeof(int), 10));
	EXPECT_NE(nullptr, queue);
	EXPECT_EQ(0, pal_queue_enqueue(queue, &itemToAdd, PAL_OS_NO_TIMEOUT));
	EXPECT_EQ(1, pal_queue_get_items(queue));
	EXPECT_EQ(9, pal_queue_get_free_slots(queue));
	EXPECT_EQ(0, pal_queue_dequeue(queue, &retrievedItem, PAL_OS_NO_TIMEOUT));
	pal_queue_destroy(&queue);
	EXPECT_EQ(nullptr, queue);
}
...
TEST(pal_os_queue, DequeAlreadyFilledQueueWithTimeout)
{
	...
	EXPECT_EQ(0, pal_queue_enqueue(queue, &itemToAdd, 2000));
	stop_time = time(NULL);
	EXPECT_EQ(0, stop_time - start_time);
	EXPECT_EQ(1, pal_queue_get_items(queue));
	EXPECT_EQ(9, pal_queue_get_free_slots(queue));
	start_time = time(NULL);
	EXPECT_EQ(0, pal_queue_dequeue(queue, &retrievedItem, 2000));
	stop_time = time(NULL);
	EXPECT_EQ(0, stop_time - start_time);
	pal_queue_destroy(&queue);
	EXPECT_EQ(nullptr, queue);
...
TEST(pal_os_queue, DequeFailedNoitemsNoTimeout)
{
	...
	EXPECT_EQ(0, pal_queue_get_items(queue));
	EXPECT_EQ(10, pal_queue_get_free_slots(queue));
	EXPECT_EQ(-1, pal_queue_dequeue(queue, &retrievedItem, PAL_OS_NO_TIMEOUT));
	pal_queue_destroy(&queue);
	EXPECT_EQ(nullptr, queue);
}
...
TEST(pal_os_queue, DequeFailedNoitemsWithTimeout)
{
	...
	EXPECT_EQ(10, pal_queue_get_free_slots(queue));
	start_time = time(NULL);
	EXPECT_EQ(-1, pal_queue_dequeue(queue, &retrievedItem, 2000));
	stop_time = time(NULL);
...
TEST(pal_os_queue, EnqueFailFilledQueueNoTimeout)
{
	...
	EXPECT_EQ(-1, pal_queue_enqueue(queue, &itemToAdd, PAL_OS_NO_TIMEOUT));
	pal_queue_destroy(&queue);
	EXPECT_EQ(nullptr, queue);
}
...
TEST(pal_os_queue, EnqueFailFilledQueueWithTimeout)
{
	...
	start_time = time(NULL);
	EXPECT_EQ(-1, pal_queue_enqueue(queue, &itemToAdd, 2000));
	stop_time = time(NULL);
	EXPECT_EQ(2, stop_time - start_time);
	pal_queue_destroy(&queue);
	EXPECT_EQ(nullptr, queue);
...
TEST(pal_os_queue, ResetQueue)
{
	...
	EXPECT_EQ(6, pal_queue_get_items(queue));
	...
	EXPECT_EQ(5, pal_queue_get_items(queue));
